...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`localhost`*/ /*!50003 FUNCTION `genera_modelo_182`(Ejercicio INT) RETURNS varchar(8192) CHARSET latin1
BEGIN
...
-- IMPORTE: PARTE ENTERA
 	IFNULL(								
      ...
      AND especie=0 
      AND fecha_pago BETWEEN STR_TO_DATE(CONCAT('01,1,','2009'),'%d,%m,%Y') AND STR_TO_DATE(CONCAT('31,12,','2099'),'%d,%m,%Y') 
      ),
 			11,'0'),
 		REPEAT('0',11) ),
-- IMPORTE PARTE FRACCIONARIA
    ...
    AND especie=0 
    AND fecha_pago BETWEEN STR_TO_DATE(CONCAT('01,1,','2009'),'%d,%m,%Y') AND STR_TO_DATE(CONCAT('31,12,','2011'),'%d,%m,%Y')),'00'),
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`localhost`*/ /*!50003 FUNCTION `xml_dame_pago_de_un_socio`(socio_id INT) RETURNS varchar(8192) CHARSET latin1
BEGIN
...
DECLARE Tmp Varchar(8192);
  ...
  (select group_concat(
    '<pago>',
      '<concepto>',ifnull(concepto,''),'</concepto>',
      '<fecha_emision>',ifnull(fecha_emision,''),'</fecha_emision>',
      '<fecha_pago>',ifnull(fecha_pago,''),'</fecha_pago>',
      '<especie>',ifnull(especie,0),'</especie>',
      '<importe>',ifnull(importe,0),'</importe>',
      ifnull(concat('<comentario>',comentario,'</comentario>'),''), 
    '</pago>'
    )
  FROM pago_socio P 
...
/*!50003 SET sql_mode              = '' */ ;
DELIMITER ;;
/*!50003 CREATE*/ /*!50020 DEFINER=`root`@`localhost`*/ /*!50003 FUNCTION `xml_socios`() RETURNS varchar(8192) CHARSET latin1
BEGIN
...
-- NATURALEZA SOCIO
    'naturaleza_socio_id="',IFNULL((select n.naturaleza from naturaleza_socio n WHERE n.id=s.naturaleza_socio_id),''),'" ',
-- CERRAR EL SOCIO
    ' >',
-- PAGOS
...
-- Dump completed on 2010-05-18 16:12:55
