...
class TipoAgenteController < ApplicationController
  ...
  def listado
    @tipos = @paginado =  TipoAgente.order("nombre").paginate(:page => params[:page], :per_page => (session[:por_pagina] or 20) )
  end

  def editar_nuevo
    @tipo = TipoAgente.find_by_id(params[:id]) || TipoAgente.new
  ...
  def modificar_crear
    ...
    if @tipo.errors.empty?
      render(:update) { |page|   page.modificar :update => params[:update], :partial => "tipo_agente" , :mensaje => { :errors => @tipo.errors } } if params[:id]
  ...
  def eliminar
    @tipo = TipoAgente.find_by_id(params[:id]).destroy 
