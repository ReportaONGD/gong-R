...
module Cucumber
  module FactoryEs
    class << self
      ...
      ATTRIBUTES_PATTERN = '( con el .+?)?( (?:which|que|that) es .+?)?'
      ...
      def steps
        ...
        [ { :pattern => /^"([^\"]*)" es una? (.+?)( \(.+?\))?#{ATTRIBUTES_PATTERN}?$/,
            :action => lambda { |a1, a2, a3, a4, a5| Cucumber::FactoryEs.parse_named_creation(self, a1, a2, a3, a4, a5) } },
           { :pattern => /^existe una? (.+?)( \(.+?\))?#{ATTRIBUTES_PATTERN}$/,
            :action => lambda { |a1, a2, a3, a4| Cucumber::FactoryEs.parse_creation(self, a1, a2, a3, a4) } },
            { :pattern => /^una? (.+?)( \(.+?\))?#{ATTRIBUTES_PATTERN}$/,
            :action => lambda { |a1, a2, a3, a4| Cucumber::FactoryEs.parse_creation(self, a1, a2, a3, a4) } },
          { :pattern => /^hay una? (.+?)( \(.+?\))?#{ATTRIBUTES_PATTERN}$/,
            :action => lambda { |a1, a2, a3, a4| Cucumber::FactoryEs.parse_creation(self, a1, a2, a3, a4) } } ]
      end
      ...
      def parse_creation(world, raw_model, raw_variant, raw_attributes, raw_boolean_attributes)
        ...
        if raw_attributes.try(:strip).present?
          raw_attributes.scan(/(?:y|con|la|el|pero|,| )+(.*?) ("([^\"]*)"|anterior)/).each do |fragment|
            attribute = attribute_name_from_prose(fragment[0])
            value_type = fragment[1] # 'anterior' or a quoted string
        ...
        if raw_boolean_attributes.try(:strip).present?
          raw_boolean_attributes.scan(/(?:que| )*(no )?(?:que|el cual|la cual|es| )*(.+?)(?: y | pero |,|$)+/).each do |fragment|
      ...
      def create_record(model_class, variant, attributes)
        ...
        if defined?(::FactoryEs) && factory = ::FactoryEs.factories[factory_name]
          ::FactoryEs.create(factory_name, attributes)
...
Cucumber::FactoryEs.add_steps(self)
